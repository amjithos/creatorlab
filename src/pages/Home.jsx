...
uniform vec3 u_flower_color1;
uniform vec3 u_flower_color2;
uniform float u_root_thickness;
...
void main() {
    ...
    float color_mix = u_stop_randomizer[1];
    vec3 flower_color = mix(u_flower_color1, u_flower_color2, color_mix);
...
const Home = () => {
  ...
  useEffect(() => {
    let renderer, sceneShader, sceneBasic, camera, clock, renderTargets, basicMaterial, shaderMaterial;
    let animationId;
    const pointer = { x: 0.66, y: 0.3, clicked: true };
    let isTouchScreen = false;
    let isPointerOverPane = false;

    // Tweakpane parameters (default values as originally shared)
    const params = {
      flowerColor1: { r: 0.71, g: 0.08, b: 0.41 },
      flowerColor2: { r: 1.00, g: 0.43, b: 0.70 },
      rootThickness: 0.003,
    };

    // Create Tweakpane
    const pane = new Pane({ title: 'Flower Controls' });
    paneRef.current = pane;
    pane.element.style.position = 'absolute';
    pane.element.style.top = '20px';
    pane.element.style.right = '20px';
    pane.element.style.zIndex = '2000';

    // Track pointer over Tweakpane
    pane.element.addEventListener('pointerenter', () => { isPointerOverPane = true; });
    pane.element.addEventListener('pointerleave', () => { isPointerOverPane = false; });
    ...
    function createPlane() {
      shaderMaterial = new THREE.ShaderMaterial({
        uniforms: {
          ...
          u_flower_color1: { type: 'v3', value: new THREE.Vector3(params.flowerColor1.r, params.flowerColor1.g, params.flowerColor1.b) },
          u_flower_color2: { type: 'v3', value: new THREE.Vector3(params.flowerColor2.r, params.flowerColor2.g, params.flowerColor2.b) },
          u_root_thickness: { type: 'f', value: params.rootThickness },
        },
        vertexShader,
        fragmentShader,
      });
    ...
    // Add Tweakpane controls (v4 API)
    const color1 = pane.addBinding(params, 'flowerColor1', { label: 'Flower Color 1', color: { type: 'float' } });
    color1.on('change', ev => {
      shaderMaterial.uniforms.u_flower_color1.value.set(ev.value.r, ev.value.g, ev.value.b);
    });

    const color2 = pane.addBinding(params, 'flowerColor2', { label: 'Flower Color 2', color: { type: 'float' } });
    color2.on('change', ev => {
      shaderMaterial.uniforms.u_flower_color2.value.set(ev.value.r, ev.value.g, ev.value.b);
    });

    const thickness = pane.addBinding(params, 'rootThickness', { label: 'Root Thickness', min: 0.001, max: 0.01, step: 0.0005 });
    thickness.on('change', ev => {
      shaderMaterial.uniforms.u_root_thickness.value = ev.value;
    });
    ...
    function updateSize() {
      if (!shaderMaterial || !renderer) return;
    ...
    const handleClick = (e) => {
      // Prevent canvas click if pointer is over Tweakpane
    ...
    cleanBtnRef.current.addEventListener('click', cleanCanvas);
    ...
    return () => {
      ...
      if (cleanBtnRef.current) cleanBtnRef.current.removeEventListener('click', cleanCanvas);
      if (renderer) {
        renderer.dispose();
      }
      if (renderTargets) {
  ...
  return (
    <>
      <div className="container">
        ...
        <div className="webgl-fade"></div>
        <div className="name">Click To Add Flowers</div>
      </div>
